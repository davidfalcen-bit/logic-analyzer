...
ftxui::Element make_layout(const logic_an_input &input, const status_conf &status, const ftxui::Component& channel_input, const ftxui::Component& freq_input, const ftxui::Component& sample_input){
    ...
    auto left_box = ftxui::vbox({ftxui::center(ftxui::bold(ftxui::text("Logic Analyzer"))), ftxui::separatorDashed(), ftxui::filler(), left_content, ftxui::filler()});
    ...
    auto right_box = ftxui::vbox({ftxui::center(ftxui::bold(ftxui::text("Status"))), ftxui::separatorDashed(), ftxui::filler(), right_content, ftxui::filler()});
    right_box = sizing(right_box, 60, 7);

    
    auto high_panel = ftxui::hbox({left_box, ftxui::separatorEmpty(), right_box});
    auto control_content = ftxui::vbox({
    ...
    auto control_box = ftxui::vbox({
        ftxui::center(ftxui::bold(ftxui::text("Controls"))),
